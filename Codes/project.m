...
%===================================
win = 25;												%Lenght of the windown patch.
winby2 = floor(win/2);
inp = imread(image);
sz = size(inp);
...
for i= winby2 +1: winby2 +sz(2)
	for j = winby2+1:winby2+sz(1)
        m = repmat(tmag(i,j),win,win);
        w_s = ones(win,win);
		w_m = (tmag(i-winby2:i+winby2,j-winby2:j+winby2)- m + ones(win,win))/2;
...
normfact = max(sxy(:));
%normfact = mean(sxy(:));
normfact = 0.3*(normfact.^2);
t_next(:,:,1) = t_next(:,:,1)/normfact;
t_next(:,:,2) = t_next(:,:,2)/normfact;
[tmag1 ,tdir1] = imgradient(t_next(:,:,1),t_next(:,:,2));
dir = atan2(t_next(:,:,2),t_next(:,:,1));
imshow(tdir1);
